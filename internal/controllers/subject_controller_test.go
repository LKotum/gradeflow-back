...
type subjTestCtx struct {
	r   *gin.Engine
	jwt string
}

func setupSubjTest(t *testing.T) subjTestCtx {
	t.Helper()
	gin.SetMode(gin.TestMode)
	db, err := gorm.Open(sqlite.Open("file:subjdb?mode=memory&cache=shared"), &gorm.Config{})
	if err != nil {
	...
	must(db.Exec(`CREATE TABLE IF NOT EXISTS subjects (id TEXT PRIMARY KEY NOT NULL DEFAULT (lower(hex(randomblob(16)))), created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, deleted_at DATETIME, department_id TEXT, code TEXT UNIQUE, title TEXT, credits INTEGER)`).Error)
	must(db.Exec(`INSERT INTO users (id,email,full_name,password_hash,role,status,totp_enabled) VALUES ('u1','subj-dean@example.com','Dean','x','dean','active',0)`).Error)
	...
	r := gin.New()
	ctrl := ctr.NewSubjectController(db, cfg)
	grp := r.Group("/api")
...
func TestSubjectCRUD(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/subjects", strings.NewReader(`{"departmentId":"d1","code":"ALG","title":"Algebra","credits":3}`))
	...
	// get
	w = httptest.NewRecorder()
	req = httptest.NewRequest("GET", "/api/subjects/"+id, nil)
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
	...
	// update
	w = httptest.NewRecorder()
	req = httptest.NewRequest("PUT", "/api/subjects/"+id, strings.NewReader(`{"title":"Linear Algebra"}`))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
	...
	// delete
	w = httptest.NewRecorder()
	req = httptest.NewRequest("DELETE", "/api/subjects/"+id, nil)
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
