...
func setupGradeTest(t *testing.T) gradeTestCtx {
	t.Helper()
	gin.SetMode(gin.TestMode)
	db, err := gorm.Open(sqlite.Open("file:gradedb?mode=memory&cache=shared"), &gorm.Config{})
	if err != nil {
	...
	must(db.Exec(`INSERT INTO users (id,email,full_name,password_hash,role,status,totp_enabled) VALUES ('u1','grade-teacher@example.com','Teacher','x','teacher','active',0)`).Error)
	must(db.Exec(`INSERT INTO assessments (id,course_id,type,date_at,scale) VALUES ('a1','c1','quiz',CURRENT_TIMESTAMP,'points')`).Error)
	...
	return gradeTestCtx{r: r, jwt: s}
}

func TestGradeCRUD(t *testing.T) {
	ctx := setupGradeTest(t)
	// create
	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/api/grades", strings.NewReader(`{"assessmentId":"a1","studentId":"st1","scale":"points","valueNum":10}`))
	...
	// list
	w = httptest.NewRecorder()
	req = httptest.NewRequest("GET", "/api/grades", nil)
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	...
	// get
	w = httptest.NewRecorder()
	req = httptest.NewRequest("GET", "/api/grades/"+id, nil)
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
	...
	// update
	w = httptest.NewRecorder()
	req = httptest.NewRequest("PUT", "/api/grades/"+id, strings.NewReader(`{"valueNum":9.5}`))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
	...
	// delete
	w = httptest.NewRecorder()
	req = httptest.NewRequest("DELETE", "/api/grades/"+id, nil)
	req.Header.Set("Authorization", "Bearer "+ctx.jwt)
	ctx.r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
